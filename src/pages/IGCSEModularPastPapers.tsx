...
const IGCSEModularPastPapers = () => {
  ...
  useEffect(() => {
    const loadSubjects = async () => {
      ...
      setError("");
      try {
        setSubjects(await fetchPastPaperSubjects("igcse-modular"));
      } catch (err) {
        ...
        <div className="container mx-auto px-4">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="text-center mb-16">
            <span className="inline-block px-4 py-1 rounded-full bg-primary/10 text-primary font-medium text-sm mb-4">
              IGCSE Modular Past Papers
            </span>
            <h1 className="text-3xl md:text-4xl lg:text-5xl font-heading font-bold text-[#1E3A8A] mb-4">
              IGCSE Modular Past <span className="inline-block px-3 py-1 border-2 border-[#FACC15] rounded-lg">Papers</span>
            </h1>
            <p className="text-muted-foreground max-w-2xl mx-auto text-lg">
              Access past examination papers and mark schemes for all IGCSE Modular subjects.
            </p>
          </motion.div>
          ...
          <div className="max-w-2xl mx-auto space-y-3">
            {loading ? (
              ...
              <p className="rounded-xl border bg-card p-6 text-center text-muted-foreground">No IGCSE Modular past papers have been added yet.</p>
            ) : (
              subjects.map((subject, index) => (
                <Link key={subject.slug} to={`/igcse-modular-past-papers/${subject.slug}`}>
...
export default IGCSEModularPastPapers;
